...
public class RestApi {
    ...
    public String get(String url, JSONObject payload) {
        ...
        if (url.equals("/users")) {
            return jsonObjectUsers(usersArray).toString();
        }
        return "[]";
    }
    ...
    public String post(String url, JSONObject payload) {
        if (url.equals("/add")) {
            ...
            if (this.lender.compareTo(this.borrower) < 0) {
                return jsonObjectUsers(new JSONArray().put(this.lender).put(this.borrower)).toString();
            } else {
                return jsonObjectUsers(new JSONArray().put(this.borrower).put(this.lender)).toString();
            }
    ...
    private JSONObject jsonObjectUser(User user) {
        List<Iou> owes = new ArrayList<>(user.owes());
        List<Iou> owesBy = new ArrayList<>(user.owedBy());
        ...
        if (lender.length() > 0 && user.name().equals(lender)) {
            for (Iou owe : owes) {
                if (owe.name.equals(borrower)) {
                    newAmount = owe.amount - this.amount;
                    if (newAmount > 0.0) {
                        hasPay = true;
            ...
            if (newAmount < 0.0){
                newOwesBy.add(new Iou(borrower, newAmount*-1.0));
            } else if (!hasBorrower && !hasPay) {
                newOwesBy.add(new Iou(borrower, amount));
            } 
        }
        newAmount = 0.0;
        hasPay = false;
        if (borrower.length() > 0 && user.name().equals(borrower)) {
            ...
            if (newAmount <0.0 ){
                newOwes.add(new Iou(lender, newAmount*-1.0));
            }else if (!hasLender && !hasPay){
                newOwes.add(new Iou(lender, amount));
            }
        }
        if (!newOwes.isEmpty()) {
            for (Iou owe : newOwes) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            for (User user : users) {
                if (user.name().equals(name)) {
                    jsonArrayUsers.put(jsonObjectUser(user));
                }
            }
        }
